...
public class GridManager {
	...
	public GridManager(PApplet p, InteractiveMap m,
			ArrayList<DataCrashInstance> results2) {
		parent = p;
		map = m;
		results = results2;
		...
		for (int i = 0; i < pies.length; i++) {
			for (int j = 0; j < pies[i].length; j++) {
				for (int k = 0; k < pies[i][j].length; k++) {
					pies[i][j][k] = new Piechart(parent,
							Utilities.Converter(20),
							(float) (Utilities.mapOffset.x + (j + 0.5)
									* gridHStep),
							(float) (Utilities.mapOffset.y + (i + 0.5)
									* gridVStep));
	...
	public void computeGridValues() {
		...
		System.out.println("Looking at " + locs[0][0].lat + " "
				+ locs[0][0].lon + " up to "
				+ locs[locs.length - 1][locs[0].length - 1].lat + " "
				+ locs[locs.length - 1][locs[0].length - 1].lon);
		;
		...
		for (DataCrashInstance dq : results) {
			...
			for (r = 0; r < locs.length - 1; r++) {
				if (locs[r + 1][0].lat <= dq.getLatitude()
						&& dq.getLatitude() <= locs[r][0].lat)
					break;
			}

			int c;
			for (c = 0; c < locs[r].length - 1; c++) {
				if (locs[r][c].lon <= dq.getLongitude()
						&& dq.getLongitude() <= locs[r][c + 1].lon)
					break;
			}
			...
			if ((r != locs.length - 1) && (c != locs[r].length - 1)) {
				...
				while (piesIndex < pc.values.size()) {
					if (Utilities.focusAttribute.equals("Age")) {
						if (pc.values.get(piesIndex).key.equals(dq.getAge())) {
							break;
						}
					} else if (Utilities.focusAttribute.equals("Month")) {
						if (pc.values.get(piesIndex).key.equals(dq.getMonth())) {
							break;
						}
					} else if (Utilities.focusAttribute.equals("Day of Week")) {
						if (pc.values.get(piesIndex).key.equals(dq.getDay_of_week())) {
							break;
						}
					} else if (Utilities.focusAttribute.equals("Light Condition")) {
						if (pc.values.get(piesIndex).key.equals(dq.getLight_condition())) {
							break;
						}
					} else if (Utilities.focusAttribute.equals("Alcohol Involved")) {
						if (pc.values.get(piesIndex).key.equals(dq.getAlchol_involved())) {
							break;
						}
					} else if (Utilities.focusAttribute.equals("Sex")) {
						if (pc.values.get(piesIndex).key.equals(dq.getSex())) {
							break;
						}
					} else if (Utilities.focusAttribute.equals("Hour")) {
						if (pc.values.get(piesIndex).key.equals(dq.getHour())) {
							break;
						}
	...
	public void drawCircles(int year) {
		...
		for (int i = 0; i < gridValues.length; i++)
			for (int j = 0; j < gridValues[i].length; j++)
				if (gridValues[i][j][yearIndex] > maxGridValue)
					maxGridValue = gridValues[i][j][yearIndex];
		...
		for (int i = 0; i < gridValues.length; i++) {
			for (int j = 0; j < gridValues[i].length; j++) {
				float diameter = maxDiam * gridValues[i][j][yearIndex]
						/ maxGridValue;
				pies[i][j][yearIndex].diameter = diameter;
				// parent.fill(0x77EE1010);
				// parent.ellipse((float)(Utilities.mapOffset.x+(j+0.5)*gridHStep),
				// (float)(Utilities.mapOffset.y+(i+0.5)*gridVStep), diameter,
				// diameter);
	...
	public void drawGrid() {
		...
		parent.stroke(Colors.gridColor);
		for (int i = 0; i < gridVLine; i++) {
			parent.line(Utilities.mapOffset.x + gridHStep * i,
					Utilities.mapOffset.y, Utilities.mapOffset.x + gridHStep
							* i, Utilities.mapOffset.y + Utilities.mapSize.y);
		}

		for (int i = 0; i < gridHLine; i++) {
			parent.line(Utilities.mapOffset.x, Utilities.mapOffset.y
					+ gridVStep * i, Utilities.mapOffset.x
					+ Utilities.mapSize.x, Utilities.mapOffset.y + gridVStep
					* i);
		}
	}
}
