...
import com.modestmaps.providers.*;
	...
	public class InteractiveMap implements PConstants {
	  ...
	  // Offset of the map (in world coordinates).
	  ...
	  /**
	   	* make a new interactive map, using the given provider, of the given width and height
	   	*/
	  public InteractiveMap(PApplet papplet, AbstractMapProvider provider, float offsetX,
		float offsetY, float width, float height) {
		this(papplet, provider, width, height);
	  ...
	  public void draw() {
	    ...
	    p.pushMatrix();
	    p.translate(offsetX+width/2, offsetY+height/2);
	    p.scale((float)sc);
	    ...
	    float screenMinX = offsetX;
	    float screenMinY = offsetY;
	    float screenMaxX = offsetX+width;
	    float screenMaxY = offsetY+height;
